...
export class AuthService {
   ...
   * 3) 아무나 접근할 수 없는 정보 (private route)를 접근할때는
  ...
  async registerWithEmail(user: RegisterUserDto) {
    const hash = await bcrypt.hash(
      user.password,
      parseInt(
        this.configService.get<string>(ENV_HASH_ROUNDS_KEY, { infer: true })!,
      ),
    );
